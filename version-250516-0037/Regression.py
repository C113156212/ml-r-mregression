...
w = np.zeros(2)    # 兩個特徵
b = 0
lr = 0.003          # 學習率*(初始設置為0.01，但太大了，使得損失函數震盪，且不收斂) --> 所以改成 0.003
epochs = 1000        # 訓練次數(初始值為1000),但無法收斂 --> 改成5000
...
# 測試資料
# 假設有一位學生，分數為 80，表現為 2
test_x = np.array([80, 2])  # 分數 75，表現 2
z = np.dot(test_x, w) + b
prob = sigmoid(z)
print("錄取機率：", prob)
print("預測結果：", int(prob >= 0.5))  # 若機率 >= 0.5 則預測為 1
